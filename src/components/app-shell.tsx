...
export function AppShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="noise relative min-h-screen overflow-hidden">
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#06101dd6] backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between md:px-6">
          <div>
            <p className="font-display text-sm uppercase tracking-[0.34em] text-mist">PropertyPro</p>
            <p className="text-sm text-fog">Autonomous property intelligence for collections, control, and cash integrity.</p>
          </div>
          <nav className="flex flex-wrap gap-2">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white/88 transition hover:border-brand hover:bg-white/10 hover:text-white"
              >
                {item.label}
              </Link>
            ))}
